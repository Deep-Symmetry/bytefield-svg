...
(def left-margin 1)
(defattrs :vertical  ;; <1>
          (eval-attribute-spec [:plain {:writing-mode "vertical-rl"}]))  ;; <2>
(draw-column-headers {:labels (str/split "7,6,5,4,3,2,1,0" #",")})
(def row-height 80)  ;; <3>
(draw-box nil)
(draw-box (text "Play" :vertical))  ;; <4>
...
as a new named attribute set.

<2> We want to combine the new CSS style attribute map `{:writing-mode
"vertical-rl"}` with the <<attrs.adoc#predefined-attributes,predefined
attributes>> named `:plain`, which are what `draw-box` normally uses.
The easiest way to do that is by
<<attrs.adoc#combining-attributes,combining attributes>> in an
attribute expression, which we can use
<<eval-attribute-spec,`eval-attribute-spec`>> to resolve for us.

<3> At the start of the row where we're using rotated text, we
...
<4> Because we want to apply our attributes to the text, rather than
...
(def left-margin 1)
(defattrs :vertical
          (eval-attribute-spec [:plain {:writing-mode "vertical-rl"}]))  ;; <1>
(draw-column-headers {:labels (str/split "7,6,5,4,3,2,1,0" #",")})
(def row-height 80)  ;; <2>
(draw-box nil)
