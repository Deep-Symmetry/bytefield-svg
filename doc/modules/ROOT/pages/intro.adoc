...
memory layouts, an similar binary structures. It sets up a drawing
...
Clojure sequence handling forms like `doseq`:
...
argument, and does the looping for you:
...
With that introduction, we're ready to start exploring all the values
that can be configured, and the full details of the functions that you
can use for drawing.
